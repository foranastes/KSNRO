...
foreach($images as $image){
	...
	$query1->select('*');
	$query1->from('#__huge_it_videogallery_videos');
	$query1 -> where('videogallery_id ='.$idofgallery);
	$query1 ->order('#__huge_it_videogallery_videos.ordering asc');
	$db1->setQuery($query1);
	$page_images1 = $db1->loadObjectList();
}
foreach($page_images1 as $page_image1){?>
var	player_<?php echo $page_image1->id;?>;
<?php } ?>
...
function onYouTubeIframeAPIReady() {
	...
	?>
	player_<?php echo $page_image1->id;?> = new YT.Player('player_<?php echo $page_image1->id;?>', {
		videoId: '<?php echo $this->get_video_gallery_id_from_url($page_image1->image_url)[0];?>',
		events: {
			'onReady': onPlayerReady<?php echo $page_image1->id;?>,
			'onStateChange': onPlayerStateChange<?php echo $page_image1->id;?>
		}
	});

// 4. The API will call this function when the video player is ready.
	function onPlayerReady<?php echo $page_image1->id;?>(event) {

	}
	function onPlayerStateChange<?php echo $page_image1->id;?>(event) {
...
jQuery(document).ready(function(){
	...
	jQuery(window).resize(function(){
		...
		<style type="text/css">
			...
			#huge_it_videogallery_container_<?php echo $videogalleryID; ?>{
				margin: 0px auto;
				width: 594px;
			}
...
<div class="itemHeader galleryhd">
...
<div id="gallerybody">
		...
		<section id="huge_it_videogallery_content_<?php echo $videogalleryID; ?>">
			<div id="huge_it_videogallery_container_<?php echo $videogalleryID; ?>" class="super-list variable-sizes clearfix">
				...
				$gallery = '';
				foreach($images as $image){
					$idofgallery=intval($image->videogallery_id );
				}
				...
				foreach($page_images as $key=>$row)
				{
					...
					?>
					<div class="element_<?php echo $videogalleryID; ?>" id="huge_it_videogallery_pupup_element_<?php echo $row->id; ?>_child" tabindex="0" data-symbol="<?php echo str_replace('__5_5_5__','%',$row->name); ?>" data-category="alkaline-earth" title="<?php echo str_replace('__5_5_5__','%',$row->name); ?>">
							...
							function getresult(pagenum,perpage,galleryid,thumbtext,linkbutton,showbutton,width,height){
								...
								jQuery.post("../components/com_videogallerylite/ajax_url.php",data,function(response){
									if(response.success){
										...
										jQuery('#huge_it_videogallery_popup_list_<?php echo $videogalleryID; ?>').append(jQuery(response.append) );
						...
						<div class="image-block_<?php echo $videogalleryID; ?>">
							...
							switch($imagerowstype){
								...
								case 'video':
									...
									$videourl=$this->get_video_gallery_id_from_url($row->image_url);
									if($videourl[1]=='youtube'){?>
										<div class="hg_iframe_class" id="player_<?php echo $row->id; ?>"></div>
...
/////////////////////////////////// VIEW 1 CONTENT SLIDER ////////////////////////////////////
	case 1;
		...
		<script>
			var slideEffect = '<?php echo $gallery[0]->sl_position;?>'.split('_');
			var catalogSliderOptions_<?php echo $videogalleryID; ?> = {

				autoSlide : <?php if($gallery[0]->autoslide == 'on') echo 'true' ; else echo 'false';?>,
				slideEaseDuration : (+'<?php echo $gallery[0]->param;?>'),
				autoSlideInterval : (+'<?php echo $gallery[0]->description;?>'),
				pauseOnHover:<?php if($gallery[0]->pause_on_hover_new == 'on') echo 'true' ; else echo 'false';?>

			}
			console.log(catalogSliderOptions_<?php echo $videogalleryID; ?>);
			jQuery('#main-slider_<?php echo $videogalleryID; ?>').liquidSlider(catalogSliderOptions_<?php echo $videogalleryID; ?>);
		</script>
...
<?php } ?>
	...
	.playbutton{
		...
		$arrowfolder=  JUri::root().'media/com_videogallerylite/Images';
        ...
        <script src="<?php echo JUri::root().'media/com_videogallerylite/js/jquery.lazyload.min.js';?>"></script>
			...
			elseif($paramssld['video_ht_view7_loadmore_position'] == 'right') { echo 'right'; }?>;
				...
				#huge_it_videogallery li img {
					...
					height: <?php echo $paramssld["thumb_image_height"] - 2*$paramssld["thumb_image_border_width"]; ?>px !important;
