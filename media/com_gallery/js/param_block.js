...
var changeParent;
var deleteCount = 1;
function Add(sel, imgTitle,parentId, isNewProject)
{
    
    if (isNewProject) {
      window.location.href='index.php?option=com_gallery&view=gallery&task=gallery.addProject&sel='+sel+'&imgTitle='+imgTitle+'&parentId='+parentId;
      return;        
    }
    ...
    if (par_images) {
        ...
        modal.setAttribute('title', imgTitle);
...
function getImage(rootUrl, parentEl, updateId_, isNewProject,edit)
{
    if(isNewProject){
        var ImageSrc = document.getElementById('_unique_name_button').value;
    }
    else if(edit){
        var ImageSrc = location.protocol+'//'+rootUrl+'/'+document.getElementById('unique_name_button'+updateId_).value;
    }
    else {
        var ImageSrc = location.protocol+'//'+rootUrl+'/'+document.getElementById('unique_name_button'+parentEl).value;
    }
    updateId = updateId_;
    ...
    var ImageTitle = '';
            ...
            if (updateId_ == null) {
               Add(ImageSrc, ImageTitle,currentParentId, isNewProject);
            } else {
                Edit(ImageSrc, ImageTitle,currentParentId, updateId );
            }
}

   //el chgres

function Edit(src,imgTitle,parentId, elementId) {
    ...
    var img_title =  document.getElementById("title"+parentId);   
        img_title.value = imgTitle;    
       // img_title.title = imgTitle;
}
